...
// Client settings
const clientSettings = { showDisabled: true, showState: false, exitOnClose: false };
{
  clientSettings.showDisabled = !!storage.get("showDisabled", true);
  clientSettings.showState = !!storage.get("showState", false);
  clientSettings.exitOnClose = !!storage.get("exitOnClose", false);
}
...
const createWindow = () => {
  ...
  win.on("close", (e) => {
    if (!forceQuit && !clientSettings.exitOnClose) {
      e.preventDefault();
      win.hide();
    }
  });
...
const setupEvents = () => {
  ...
  ipcMain.on("set-client-settings", (event, settings: { showDisabled?: boolean, showState?: boolean, exitOnClose?: boolean }) => {
    ...
    if (settings.exitOnClose !== undefined) clientSettings.exitOnClose = settings.exitOnClose;
    ...
    storage.set("exitOnClose", clientSettings.exitOnClose);
  });
